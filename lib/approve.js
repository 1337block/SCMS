...
async function ApproveAddProductToDistributor(data) {
      ...
      var distributorRegistry = await getAssetRegistry("org.scms.network.distributor.Distributor");
      var distributor = await distributorRegistry.get(data.distributor_ID);
      var numberOfItemOrdered = getNumberOfItems(distributor.reqProductList, product.product_ID);
      var numberItems = numberOfItemOrdered - data.numberOfItemsApproved;
      ...
      if (numberItems < 0) {
            throw new Error(`Distributor ordered only ${numberOfItemOrdered}`);
      }
